...
# calculate medial axis points
        ...
        if self.ui.edtSpinBox.value > 0:
            ...
            BOX_EDT = max(5,int(edt[startCandidate[0],startCandidate[1],startCandidate[2]])*2)
...
# Now I find medial axis from BFS algorithm
        ...
        #workDir = os.path.dirname(__file__)
        #f = open(workDir + '/log1.txt','a')
        #print('.....',file=f)
        #f.close()

        # This is medial axis from start point to end point
        if self.ui.dijkstraCheckBox.checked == True:
            pathStartEnd = find_path_dijkstra(edt,start,end)
            #workDir = os.path.dirname(__file__)
            #f = open(workDir + '/log2.txt','a')
            #print('BFS',file=f)
            #print(pathStartEnd,file=f)
            #f.close()
        else:
            pathStartEnd = find_path_bfs(edt,start,end)
            #workDir = os.path.dirname(__file__)
            #f = open(workDir + '/log1.txt','a')
            #print('BFS',file=f)
            #print(pathStartEnd,file=f)
        ...
        if self.ui.dijkstraCheckBox.checked == True:
            pathStartAneurysm = find_path_dijkstra(edt,aneurysmCenter,end)
            #workDir = os.path.dirname(__file__)
            #f = open(workDir + '/log2.txt','a')
            #print('BFS',file=f)
            #print(pathStartAneurysm,file=f)
            #f.close()
        else:
            pathStartAneurysm = find_path_bfs(edt,aneurysmCenter,end)
            #workDir = os.path.dirname(__file__)
            #f = open(workDir + '/log1.txt','a')
            #print('BFS',file=f)
            #print(pathStartAneurysm,file=f)
...
# For this purpose we will go iteratively along the medial axis, finding MPR at the points, and finding diameters of the cross sections
        ...
        if self.ui.numOfPointsSpinBox.value > 0:
            ...
            NPOINTS = int(len(pathStartAneurysm)/10)
            
        if self.ui.slidingWindowSpinBox.value > 0:    
            STEP = self.ui.slidingWindowSpinBox.value
        else:
            STEP = int(NPOINTS/2)
        ...
        NMAX = len(pathStartEnd)
        PIX_SIZE = imSpacing[0]  # pixel size in mm

        indexDiff = find_diff_index(pathStartEnd,pathStartAneurysm)
        ...
        points = []
        vectors = []
        for START in range(0,NMAX,STEP):
            
            pointsToFit = np.array(pathStartEnd[START:START+NPOINTS])
            ...
            # id=0 USUALLY corresponds to background
            ids = ids[1:]
            counts = counts[1:]
            idMax = ids[np.argmax(counts)]
            dum[labels!=idMax] = 0

            dum = np.asarray(dum,dtype=np.uint8)
            props = measure.regionprops(dum)
            
            min_axes.append(props[0].axis_minor_length*2*PIX_SIZE)
            max_axes.append(props[0].axis_major_length*2*PIX_SIZE)
            
            #CImg(crossSection + dum*np.max(crossSection)/2).display(f'vessel cross secrion no. {START} of {NMAX}');
            #break
            
        distances = [np.linalg.norm(P-np.array(pathStartEnd[indexDiff])) for P in points]
        minDistIndex = np.argmin(distances)

        points = np.asarray(points)
        distancesAlongPath = [-path_length(points[i:minDistIndex+1])*PIX_SIZE for i in range(minDistIndex)] + [path_length(points[minDistIndex:i+minDistIndex])*PIX_SIZE for i in range(1,points.shape[0] - minDistIndex+1)]     


        #plt.figure(figsize=(10,10))
        #plt.plot(distancesAlongPath,min_axes,'o-',label='minor diameter in mm')
        #plt.plot(distancesAlongPath,max_axes,'x-',label='major diameter in mm')
        #plt.legend()
        #plt.ylabel('diameter [mm]')
        #plt.xlabel('distance from aneurysm junction [mm]')

        saveDir = os.path.dirname(__file__)
        #plt.savefig(saveDir + '/diameters')
        ...
        #plt.figure(figsize=(10,10))
        #plt.plot(distancesAlongPath[1:],angles,'o-')
        #plt.ylabel('angle [degrees]')
        #plt.xlabel('distance from aneurysm junction [mm]')
        #plt.savefig(saveDir + '/angles')
...
# przektore przez tętniaka
        ...
        def findCrossSection(P,V):
            ...
            return crossSection, dum, props[0].axis_minor_length*2*PIX_SIZE, props[0].axis_major_length*2*PIX_SIZE
        
        v0,p0 = vectors[minDistIndex],points[minDistIndex] # wektor i punkt wzdłuż osi naczynia w miejscu połaczenia z tętniakiem

        pointsToFit = np.array(pathStartAneurysm[len(pathStartAneurysm)-NPOINTS:len(pathStartAneurysm)]) #śroedek tętniaka na koncu listy punktów
        P,V = fit_3d_line(pointsToFit)
        v1,p1 = V, pathStartAneurysm[-1] # wektor wzdłuż osi łączącej środek tętniaka z naczyniem, wzdłuż szyjki i punkt centralny tętniaka
        ...
        #fname = saveDir +  f'/cross section 1, min axis = {minAxis:.2f}, max axis = {maxAxis:.2f}.png'
        #io.imsave(fname,dum)
        #img = CImg(dum)
        #img.display(f'cross section 1, min axis = {minAxis:.2f}, max axis = {maxAxis:.2f} ');
        #img.save_png(saveDir +  f'/cross section 1, min axis = {minAxis:.2f}, max axis = {maxAxis:.2f}.png')
    ...
    #    print(minAxis,maxAxis )
        ...
        #fname = saveDir +  f'/cross section 2, min axis = {minAxis:.2f}, max axis = {maxAxis:.2f}.png'
        #io.imsave(fname,dum)
        #img = CImg(dum)
        #img.display(f'cross section 2, min axis = {minAxis:.2f}, max axis = {maxAxis:.2f} ');
        #img.save_png(saveDir +  f'/cross section 2, min axis = {minAxis:.2f}, max axis = {maxAxis:.2f}.png')
    #    print(minAxis,maxAxis )
        ...
        #fname = saveDir +  f'/cross section 3, min axis = {minAxis:.2f}, max axis = {maxAxis:.2f}.png'
        #io.imsave(fname,dum)
        #img = CImg(dum)
        #img.display(f'cross section 3, min axis = {minAxis:.2f}, max axis = {maxAxis:.2f} ');
        #img.save_png(saveDir +  f'/cross section 3, min axis = {minAxis:.2f}, max axis = {maxAxis:.2f} ');


####################################################################

        
        #imName = 'C:\\Users\\User\\Desktop\\SlicerExtensions\\data\\tetniak_cropped.nrrd'
        #im = sitk.ReadImage(imName, sitk.sitkFloat32)
        #imSpacing = im.GetSpacing()
        #imOrigin = im.GetOrigin()
        
        #startFiducialName = 'C:\\Users\\User\\Desktop\\SlicerExtensions\\data\\F_2.mrk.json'
        #with open(startFiducialName, 'r') as file:
        #    startData = json.load(file)

        #endFiducialName = 'C:\\Users\\User\\Desktop\\SlicerExtensions\\data\\F_3.mrk.json'
        #with open(endFiducialName, 'r') as file:
        #    endData = json.load(file)

        #centralFiducialName = 'C:\\Users\\User\\Desktop\\SlicerExtensions\\data\\F_4.mrk.json'
        #with open(centralFiducialName, 'r') as file:
        #    centerData = json.load(file)

        #pos1 = startData["markups"][0]["controlPoints"][0]["position"]
        
        #pos2 = endData["markups"][0]["controlPoints"][0]["position"]

        #pos3 = centerData["markups"][0]["controlPoints"][0]["position"]
        
        #workDir = os.path.dirname(__file__)
        #f = open(workDir + '/log.txt','w')
        #print(startCandidate,start,file=f)
        #print(endCandidate,end,file=f)
        #print(aneurysmCenterCandidate,aneurysmCenter,file=f)
        #print(seg.shape,inp.shape,np.unique(seg),file=f)
        #print('##############',file=f)
        #print(origin,imOrigin,spacing,imSpacing,file=f)
        #print('##############',file=f)
        #print(p1,pos1,file=f)
        #print(p2,pos2,file=f)
        #print(p3,pos3,file=f)
        #print('##################')
        #print(dir(self._parameterNode.markupEntry),file=f)
        #f.close()
        
        
        #seg = slicer.util.array(self._parameterNode.thresholdedVolume)
        
        #volumeNode = self._parameterNode.inputVolume
        #ctVolume = slicer.util.arrayFromVolume(self._parameterNode.inputVolume)
        ...
        #print(self._parameterNode.markupEntry,file=f)
        #print('####################',file=f)
        #print(self._parameterNode.markupExit,file=f)
        #print('####################',file=f)
        #print(self._parameterNode.markupCentral,file=f)
        #print('####################',file=f)
        #print(self.ui.entryPointSelector,file=f)
        #print('####################',file=f)
        #print(self.ui.exitPointSelector,file=f)
        #print('####################',file=f)
        #print(self.ui.centralPointSelector,file=f)
        #print('####################',file=f)
        #print(ctVolume.shape,file=f)        
        #f.close()
        #"""Run processing when user clicks "Apply" button."""
        
########################################################
        #histogram = np.histogram(ctVolume, bins=50)
...
########################################################
        #volumeNode = self._parameterNode.inputVolume
        #histogram = np.histogram(imArr, bins=50)

        #workDir = os.path.dirname(__file__)
        #f = open(workDir + '/log2.txt','w')
        #print(type(histogram),len(histogram),file=f)
        #print(histogram[0].shape,histogram[1].shape,file=f)
        #f.close()
        ...
        #workDir = os.path.dirname(__file__)
        #f = open(workDir + '/log2.txt','w')
        #print(dir(plotChartNode),file=f)
        #print(histogram[0].shape,histogram[1].shape,file=f)
        #f.close()
